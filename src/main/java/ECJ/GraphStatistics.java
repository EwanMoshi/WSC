...
public class GraphStatistics extends SimpleShortStatistics {
    ...
    public void postEvaluationStatistics(EvolutionState state){
        ...
        output = true; // this was not in original
        ...
        for(int x=0;x<subpops;x++)
            ...
            if (output && doSubpops)
                {
                state.output.print("Mean:   " + meanFitnessThisGen[x] + " ", statisticslog);
                state.output.print("Best Fitness of Generation:   " + bestOfGeneration[x].fitness.fitness() + " ", statisticslog);
                state.output.print("Best so far:    " + bestSoFar[x].fitness.fitness() + " ", statisticslog);
                }
        ...
        if (output && doSize)
            ...
            state.output.print("\nSize of best Ind. this Gen." + (double)(popBestOfGeneration.size()) + " " , statisticslog);                                    // size of best ind of pop this gen
            //state.output.print("Size of best Ind. so Far" + (double)(popBestSoFar.size()) + " " , statisticslog);                           // size of best ind of pop so far
            }

        // print out fitness info
        if (output)
            {
            state.output.print("\nMean:  " + popMeanFitness + " " , statisticslog);                                                                                  // mean fitness of pop this gen
            state.output.print("\nBest Fitnes:   " + (popBestOfGeneration.fitness.fitness()) + " " , statisticslog);                 // best fitness of pop this gen
            //state.output.print("\nBest Fitness so far:    " + (popBestSoFar.fitness.fitness()) + " " , statisticslog);                // best fitness of pop so far
            }
        ...
        // we're done!
        if (output) state.output.println("\n\n\n", statisticslog);
